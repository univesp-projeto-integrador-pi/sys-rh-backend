...
export function authMiddleware(
  ...
  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    console.warn(
      "[AuthMiddleware] Tentativa de acesso sem token ou formato inválido.",
    );
    return res
      .status(401)
  ...
  try {
    ...
    if (!secret) {
      console.error(
        "[AuthMiddleware] CRÍTICO: JWT_ACCESS_SECRET não definido no .env",
      );
      return res
        .status(500)
        .json({ message: "Erro interno de configuração no servidor" });
    }
